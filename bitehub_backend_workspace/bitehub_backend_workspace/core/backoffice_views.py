...
def _find_wallet_by_identifier(raw_identifier: str | None) -> Wallet | None:
    ...
    return _student_wallets_queryset().filter(
        Q(link_code__iexact=identifier)
        | Q(nfc_card_uid__iexact=identifier)
    ).first()
...
def cafe_panel(request: HttpRequest) -> HttpResponse:
    ...
    context = {
        ...
        "wallet_kpis": {
            ...
            "linked_cards": _student_wallets_queryset()
            .exclude(nfc_card_uid__isnull=True)
            .exclude(nfc_card_uid="")
            .count(),
