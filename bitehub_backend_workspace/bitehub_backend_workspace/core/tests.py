...
class AppAuthApiTests(TestCase):
    def test_student_signup_accepts_name_and_email(self):
        response = self.client.post(
            reverse("v2_app_signup"),
            data={
                "full_name": "طالب تجربة",
                "email": "student.signup@example.com",
        ...
        user = get_user_model().objects.get(email="student.signup@example.com")
        self.assertEqual(user.full_name, "طالب تجربة")
        self.assertTrue(Wallet.objects.filter(user=user).exists())

    def test_student_signup_rejects_missing_name_with_clear_message(self):
        response = self.client.post(
            reverse("v2_app_signup"),
            data={
                "full_name": "",
        ...
        self.assertEqual(response.json()["error"], "اسم الطالب مطلوب.")

    def test_student_signup_rejects_duplicate_email_with_clear_message(self):
        User = get_user_model()
        ...
        self.assertEqual(response.status_code, 400, response.content)
        self.assertEqual(response.json()["error"], "البريد الإلكتروني مسجل مسبقاً.")

    def test_student_can_delete_own_account_from_app_profile(self):
        User = get_user_model()
        ...
        response = self.client.delete(reverse("v2_app_user_profile"))

        self.assertEqual(response.status_code, 204, response.content)
        self.assertFalse(User.objects.filter(pk=user.pk).exists())

    def test_cafe_owner_cannot_delete_account_from_app_profile(self):
        User = get_user_model()
        ...
        Cafe.objects.create(name="Delete Guard Cafe", code="delete-guard", owner=owner)
...
class CafeProvisioningTests(TestCase):
    ...
    def test_super_admin_create_cafe_assigns_manager_runtime_access(self):
        ...
        self.assertFalse(Wallet.objects.filter(user=manager).exists())
    ...
    def test_super_admin_delete_cafe_requires_code_confirmation(self):
        User = get_user_model()
        superuser = User.objects.create_superuser(
            email="root-delete-confirm@example.com",
            password="StrongPass123",
            full_name="Root Admin",
            phone_number="0911000030",
        )
        cafe = provision_cafe(name="Confirmation Cafe", code="confirmation-cafe")
        self.client.force_login(superuser)

        response = self.client.post(
            reverse("core:delete_cafe_from_dashboard", args=[cafe.id]),
            data={"confirmation": "wrong-code"},
            follow=True,
        )

        self.assertEqual(response.status_code, 200)
        self.assertTrue(Cafe.objects.filter(pk=cafe.id).exists())

    def test_super_admin_delete_legacy_blank_code_cafe_requires_fallback_token(self):
        User = get_user_model()
        superuser = User.objects.create_superuser(
            email="root-delete-blank-code@example.com",
            password="StrongPass123",
            full_name="Root Admin",
            phone_number="0911000034",
        )
        cafe = Cafe.objects.create(name="Legacy Blank Code Cafe", code="")
        self.client.force_login(superuser)

        empty_confirmation_response = self.client.post(
            reverse("core:delete_cafe_from_dashboard", args=[cafe.id]),
            data={"confirmation": ""},
            follow=True,
        )

        self.assertEqual(empty_confirmation_response.status_code, 200)
        self.assertTrue(Cafe.objects.filter(pk=cafe.id).exists())

        delete_response = self.client.post(
            reverse("core:delete_cafe_from_dashboard", args=[cafe.id]),
            data={"confirmation": f"DELETE-{cafe.id}"},
            follow=True,
        )

        self.assertEqual(delete_response.status_code, 200)
        self.assertFalse(Cafe.objects.filter(pk=cafe.id).exists())

    def test_super_admin_can_delete_cafe_permanently_from_dashboard(self):
        User = get_user_model()
        superuser = User.objects.create_superuser(
            email="root-delete@example.com",
            password="StrongPass123",
            full_name="Root Admin",
            phone_number="0911000031",
        )
        owner = User.objects.create_user(
            email="delete-cafe@bitehub.local",
            password="CafePass123",
            full_name="Delete Cafe Operator",
            phone_number="0911000032",
            is_staff=True,
        )
        ...
        cafe = provision_cafe(name="Delete Cafe", code="delete-cafe", owner_id=owner.id)
        category = Category.objects.create(cafe=cafe, name="Delete Drinks")
        ...
        notification = Notification.objects.create(
            ...
            order=order,
            title="Delete order",
            body="Order will be removed with cafe.",
        )
        owner_id = owner.id
        ...
        self.assertFalse(OrderItem.objects.filter(pk=order_item.id).exists())
