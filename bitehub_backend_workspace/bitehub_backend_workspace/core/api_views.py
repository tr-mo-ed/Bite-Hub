...
def get_products(request):
    ...
    elif not Cafe.objects.filter(pk=cafe_id, is_active=True).exists():
        return Response([])
...
def get_user_profile(request):
    ...
    full_name_supplied = 'full_name' in request.data or 'name' in request.data
    full_name = (request.data.get('full_name') or request.data.get('name') or request.user.full_name or '').strip()
    email_supplied = 'email' in request.data
    email = (request.data.get('email') or '').strip().lower()
    phone_supplied = 'phone_number' in request.data or 'phone' in request.data
    ...
    if full_name_supplied and not full_name:
    ...
    if phone_supplied:
        ...
        if request.user.secondary_phone_number == phone_number:
            request.user.secondary_phone_number = None
...
def create_order(request):
    ...
    nfc_card_uid = request.data.get('nfc_card_uid') or request.data.get('card_uid') or ''
